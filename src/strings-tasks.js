...
function isPalindrome(str) {
  const re = /[a-zA-Z]/g;
  if (str.toLowerCase().match(re)) {
    const arr = str.toLowerCase().match(re);
    const half = Math.floor(arr.length / 2);
    const str1 = arr.join('').slice(half);
    const str2 = arr.reverse().join('').slice(half);
    if (str1 === str2) {
      return true;
    }
  }
